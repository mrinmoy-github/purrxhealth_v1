...
@Component({
  ...
  styleUrls: ['./update-billing-info.component.css']
})
export class UpdateBillingInfoComponent implements OnInit {
  ...
  onSubmit(f: NgForm) {
    ...
    this.backendapiService.commonDataPostFn('admin/order/updateBillingInfoForm', this.model).subscribe(
      resp => {
  ...
  ngOnInit() {
    ...
    this.titleService.setTitle(this.siteName+' | Update Order Billing Info');
  }
